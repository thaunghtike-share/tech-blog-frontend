...
export default function AuthorsPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 relative overflow-x-hidden">
      ...
      <MinimalHeader />
      <section className="bg-white/90 backdrop-blur-sm py-12 border-b border-gray-200/50 relative z-10">
            ...
            {authors.map((author) => (
              <Card key={author.id} className="border-0 bg-white shadow-sm hover:shadow-md transition cursor-pointer">
                <CardContent className="flex flex-col items-center space-y-4 p-6">
                  ...
                  {author.bio && <p className="text-center text-gray-500 text-sm whitespace-pre-line">{author.bio}</p>}
                  {author.linkedin && (
                    <a
